...
namespace SceneTalkVR.AvatarSystem
{
    ...
    public sealed class PilotEmbodimentPresenter : MonoBehaviour, ISceneTalkSessionReset
    {
        ...
        public bool Configure(PilotPresentationProfile value, PilotAudioSourcePolicy voiceOnlyPolicy, bool lockedPilot, out string error)
        {
            ...
            {
                humanoid=Instantiate(value.visualPrefab,transform,false);humanoid.name="Pilot Humanoid Feedback Agent";humanoid.transform.localPosition=value.sourcePosition;humanoid.transform.localRotation=Quaternion.Euler(0,180,0);humanoid.transform.localScale=Vector3.one;humanoidAnimator=humanoid.GetComponentInChildren<Animator>();humanoid.SetActive(false);
            }
        ...
        public void BeginFeedback()
        {
            ...
            if(humanoid!=null){humanoid.SetActive(true);if(humanoidAnimator!=null)humanoidAnimator.SetBool("Speaking",true);}
        }
        public void EndFeedback(){if(orb!=null){orb.EndSpeaking();orb.HideImmediate();}if(humanoid!=null){if(humanoidAnimator!=null)humanoidAnimator.SetBool("Speaking",false);humanoid.SetActive(false);}}
