...
namespace SceneTalkVR.Core
{
    ...
    public sealed class PilotWorkflowCoordinator : MonoBehaviour, ISceneTalkSessionReset
    {
        ...
        private void Awake(){if(conditionManager==null)conditionManager=GetComponent<ExperimentConditionManager>();if(presenter==null)presenter=GetComponent<PilotEmbodimentPresenter>()??gameObject.AddComponent<PilotEmbodimentPresenter>();goals.OnGoalSequenceStateChanged+=OnGoalSequenceStateChanged;Active=this;}
        private void OnDestroy(){goals.OnGoalSequenceStateChanged-=OnGoalSequenceStateChanged;if(Active==this)Active=null;}
        ...
        private void OnGoalSequenceStateChanged(GoalProgressChangedEvent value)
        {
            if(value==null||value.sequenceState==GoalSequenceState.Inactive||value.conditionRunId!=PilotRunId)return;
            var type=value.sequenceState==GoalSequenceState.AwaitingParticipantTurn?"PilotGoalAwaitingParticipantTurn":value.sequenceState==GoalSequenceState.AwaitingAvatarReply?"PilotGoalAwaitingAvatarReply":value.sequenceState==GoalSequenceState.Completed?"PilotGoalSequenceCompleted":!string.IsNullOrWhiteSpace(value.unlockTurnId)?"PilotGoalSequenceAdvanced":"PilotGoalActivated";
            Write(type,reason:$"state={value.sequenceState};index={value.activeGoalIndex};revision={value.sequenceRevision}",goalId:value.activeGoalId,turnId:value.unlockTurnId);
        }
            ...
            string evaluatorVersion="",string evaluationReason="")
        {
            ...
            var r = new PilotEventRecord
            {
                ...
                goalSequencePolicy=goals.Context.sequencePolicy.ToString(),goalSequenceState=goals.SequenceState.ToString(),
                activeGoalIndex=goals.ActiveGoalIndex,goalSequenceRevision=goals.SequenceRevision,goalUnlockTurnId=turnId??"",
