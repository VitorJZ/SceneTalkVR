...
namespace SceneTalkVR.Runtime
{
    ...
    public sealed class PilotCollectionParticipantUi : MonoBehaviour
    {
        ...
        private void Build()
        {
            ...
            Label(setup.transform,"Title","预实验会话设置",new Vector2(0,205),new Vector2(650,48),30);
            Label(setup.transform,"ParticipantLabel","参与者编号",new Vector2(-235,112),new Vector2(190,38),19,TextAnchor.MiddleLeft);
            participantInput=Input(setup.transform,"PilotParticipantIdInput","必填",new Vector2(100,112),new Vector2(400,48));
            Label(setup.transform,"SessionLabel","会话编号",new Vector2(-235,48),new Vector2(190,38),19,TextAnchor.MiddleLeft);
            sessionInput=Input(setup.transform,"PilotSessionIdInput","可选——留空则自动生成",new Vector2(100,48),new Vector2(400,48));
            ...
            Button(setup.transform,"CreatePilotSessionButton","创建预实验会话",new Vector2(-175,-105),()=>Arm(false),new Vector2(260,48));
            Button(setup.transform,"ResumePilotSessionButton","恢复预实验会话",new Vector2(175,-105),()=>Arm(true),new Vector2(260,48));
            Button(setup.transform,"PilotSetupBackButton","返回",new Vector2(0,-180),()=>coordinator.EndSession(),new Vector2(180,44));

            selection=Panel("PilotAppearanceSelectionPanel",new Vector2(820,520));
            Label(selection.transform,"Title","选择反馈角色外观",new Vector2(0,210),new Vector2(720,46),29);
            Label(selection.transform,"Body","请以任意顺序完成三种外观；已完成的外观不能再次选择。",new Vector2(0,166),new Vector2(700,50),17);
            ...
            Label(ranking.transform,"Instruction","请为每种反馈体验指定唯一排名，1 表示最喜欢。",new Vector2(0,198),new Vector2(790,34),18);
            ...
            for(var i=0;i<values.Length;i++){var condition=values[i];var y=130-i*72;Label(ranking.transform,condition+"Label",Friendly(condition),new Vector2(-245,y),new Vector2(360,42),19,TextAnchor.MiddleLeft);ranks[condition]=0;for(var rankValue=1;rankValue<=3;rankValue++){var captured=rankValue;var button=Button(ranking.transform,condition+"Rank"+rankValue,rankValue.ToString(),new Vector2(25+rankValue*58,y),()=>SelectRank(condition,captured),new Vector2(46,42));rankButtons[condition+":"+rankValue]=button;}Button(ranking.transform,condition+"Preferred","首选",new Vector2(315,y),()=>SelectPreferred(condition),new Vector2(126,42));}
            reasonInput=Input(ranking.transform,"PilotRankingReason","请说明你更喜欢该反馈体验的原因。",new Vector2(0,-125),new Vector2(720,60));
            rankingError=Label(ranking.transform,"Validation","",new Vector2(0,-188),new Vector2(760,32),17);
            Button(ranking.transform,"PilotRankingSubmitButton","提交排序",new Vector2(0,-242),SubmitRanking,new Vector2(220,46));

            completion=Panel("PilotExperimentCompletionPanel",new Vector2(700,300));
            Label(completion.transform,"Title","预实验已完成",new Vector2(0,65),new Vector2(620,50),30);
            Label(completion.transform,"Message","感谢参与，你已完成预实验。",new Vector2(0,-20),new Vector2(600,80),22);
            Button(completion.transform,"PilotCompletionContinueButton","继续",new Vector2(0,-105),ContinueAfterCompletion,new Vector2(210,48));
        ...
        private void SkipQuestionnaire()
        {
            if(!questionnaireSkipConfirmationArmed)
            {
                ...
                questionnaireError.text=SceneTalkUiText.Text("跳过将保留已填写内容并继续实验，请再次点击“确认跳过”。");
                questionnaireSkipButton.GetComponentInChildren<TMP_Text>().text=SceneTalkUiText.Text("确认跳过");
                return;
            }
        ...
        private void ResetQuestionnaireSkipConfirmation()
        {
            ...
            if(questionnaireSkipButton!=null)questionnaireSkipButton.GetComponentInChildren<TMP_Text>().text=SceneTalkUiText.Text("跳过");
        }
        ...
        private void SubmitRanking(){if(ranks.Values.Any(x=>x<1||x>3)||ranks.Values.Distinct().Count()!=3){rankingError.text=SceneTalkUiText.Text("请将每个排名各使用一次。");return;}if(!preferred.HasValue){rankingError.text=SceneTalkUiText.Text("请选择整体最喜欢的反馈体验。");return;}if(string.IsNullOrWhiteSpace(reasonInput.text)){rankingError.text=SceneTalkUiText.Text("请说明原因。");return;}var entries=ranks.Select(x=>new PreferenceRankEntry{embodimentCondition=PilotProtocolValues.Label(x.Key),rank=x.Value}).OrderBy(x=>x.rank).ToArray();var response=new PreferenceRankingResponse{rankings=entries,preferredEmbodimentCondition=PilotProtocolValues.Label(preferred.Value),reason=reasonInput.text.Trim()};if(!coordinator.SubmitFinalRanking(response,out var error))rankingError.text=Humanize(error);}
        ...
        private void Refresh()
        {
            ...
            if(stage==PilotParticipantStage.TaskIntroduction&&builtTaskPosition!=coordinator.CurrentPosition){builtTaskPosition=coordinator.CurrentPosition;var task=coordinator.CurrentTask;if(task!=null)introductionText.text=SceneTalkUiText.TaskName(task.taskId,task.displayName)+"\n\n"+SceneTalkUiText.TaskContext(task.taskId,task.context)+"\n\n"+SceneTalkUiText.Select("沟通目标：","Communication goals:")+"\n"+string.Join("\n",task.goals.Select(x=>"• "+SceneTalkUiText.Goal(x.goalId,x.text)));}
        ...
        private void RefreshAppearanceSelection(){foreach(var pair in appearanceButtons){var item=coordinator?.Assignment?.conditions?.FirstOrDefault(x=>x.embodimentCondition==pair.Key);var selectable=item!=null&&(item.status==PilotRunStatus.Assigned||item.status==PilotRunStatus.TechnicalInvalid);pair.Value.interactable=selectable;if(appearanceStatuses.TryGetValue(pair.Key,out var label))label.text=SceneTalkUiText.Text(item==null?"不可用":item.status==PilotRunStatus.Completed?"已完成":item.status==PilotRunStatus.TechnicalInvalid?"可重试":selectable?"可选择":"进行中");}}
        ...
        private static string Friendly(PilotEmbodimentCondition value)=>value==PilotEmbodimentCondition.VoiceOnly?SceneTalkUiText.Select("仅语音反馈","Voice-only feedback"):value==PilotEmbodimentCondition.FloatingOrb?SceneTalkUiText.Select("悬浮球反馈","Floating-orb feedback"):SceneTalkUiText.Select("人形辅助角色反馈","Humanoid-assistant feedback");
        private static string Humanize(string error)=>SceneTalkUiText.Error(error);
