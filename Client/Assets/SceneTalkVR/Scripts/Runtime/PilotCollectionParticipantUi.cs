...
namespace SceneTalkVR.Runtime
{
    ...
    public sealed class PilotCollectionParticipantUi : MonoBehaviour
    {
        ...
        private InputField participantInput,sessionInput,reasonInput; private Text setupError,introductionText,questionnaireError,rankingError;
        ...
        private void SelectRank(PilotEmbodimentCondition condition,int rank){ranks[condition]=rank;foreach(var pair in rankButtons)pair.Value.GetComponent<Image>().color=pair.Key==condition+":"+rank?new Color(.12f,.68f,.34f,1):new Color(.12f,.38f,.62f,1);rankingError.text="";}
        ...
        private static InputField Input(Transform parent,string name,string placeholder,Vector2 pos,Vector2 size){var go=Node(parent,name);go.AddComponent<Image>().color=new Color(.12f,.16f,.22f,1);var input=go.AddComponent<InputField>();go.GetComponent<RectTransform>().anchoredPosition=pos;go.GetComponent<RectTransform>().sizeDelta=size;var text=Label(go.transform,"Text","",Vector2.zero,size-new Vector2(20,8),18,TextAnchor.MiddleLeft);var hint=Label(go.transform,"Placeholder",placeholder,Vector2.zero,size-new Vector2(20,8),17,TextAnchor.MiddleLeft);hint.color=new Color(.65f,.7f,.75f,1);input.textComponent=text;input.placeholder=hint;return input;}
