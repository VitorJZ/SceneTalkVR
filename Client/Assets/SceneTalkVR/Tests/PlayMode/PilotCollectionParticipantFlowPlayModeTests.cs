...
namespace SceneTalkVR.Tests.PlayMode
{
    ...
    {
        [UnitySetUp]public IEnumerator SetUp(){ForcePicoDeviceValidation(false);if(SceneManager.GetActiveScene().name!="SampleScene"){SceneManager.LoadScene("SampleScene");yield return null;}yield return null;CallActive("SceneTalkVR.Core.EditorCollectionSessionCoordinator, Assembly-CSharp","EndRuntimeSession");CallActive("SceneTalkVR.Core.RehearsalSessionCoordinator, Assembly-CSharp","ResetSession");}
        [UnityTearDown]public IEnumerator TearDown(){CallActive("SceneTalkVR.Core.PilotCollectionSessionCoordinator, Assembly-CSharp","ResetPilotSessionForQa");ForcePicoDeviceValidation(false);yield return null;}
        [UnityTest]public IEnumerator MainMenu_HasIndependentFormalAndPilotRoutes()
        {Assert.That(Label("StartButton"),Is.EqualTo("Formal Experiment"));Assert.That(Label("PilotExperimentButton"),Is.EqualTo("Pilot Experiment"));AssertExitOverlay();Click("PilotExperimentButton");yield return null;Assert.That(Active("PilotInstructionsPanel"),Is.True);Assert.That(Active("PilotSessionSetupPanel"),Is.False);Assert.That(Active("FormalModeSelectionPanel"),Is.False);Assert.That(Active("TaskSelectionPanel"),Is.False);AssertExitOverlay();}
        [UnityTest]public IEnumerator PilotCreateSession_PersistsBalancedAssignmentAndShowsInstructions()
        {Click("PilotExperimentButton");yield return null;var coordinator=ActiveObject("SceneTalkVR.Core.PilotCollectionSessionCoordinator, Assembly-CSharp");Assert.That((bool)Get(coordinator,"IsArmed"),Is.True);Assert.That(Get(coordinator,"Stage").ToString(),Is.EqualTo("Instructions"));Assert.That((string)Get(coordinator,"ParticipantId"),Does.StartWith("PILOT-P-"));Assert.That((string)Get(coordinator,"SessionId"),Does.StartWith("PILOT-S-"));var assignment=Get(coordinator,"Assignment");Assert.That((bool)Get(assignment,"collectionEligible"),Is.True);Assert.That((bool)Get(assignment,"developerTestAssignment"),Is.False);var conditions=((IEnumerable)Get(assignment,"conditions")).Cast<object>().ToArray();Assert.That(conditions.Select(x=>Get(Get(x,"task"),"taskId")).Distinct().Count(),Is.EqualTo(3));Assert.That(conditions.Select(x=>Get(x,"embodimentCondition")).Distinct().Count(),Is.EqualTo(3));Assert.That(System.IO.File.Exists(System.IO.Path.Combine((string)Get(coordinator,"CurrentDataFolder"),"pilot_assignment.json")),Is.True);Assert.That(Active("PilotInstructionsPanel"),Is.True);}
        [UnityTest]public IEnumerator BeginPilot_ShowsAssignedTaskWithoutEmbodimentMetadata()
        {Create();Click("BeginPilotButton");yield return null;Assert.That(Active("PilotConditionTaskIntroductionPanel"),Is.True);var text=Text("PilotConditionTaskIntroductionPanel");Assert.That(text,Does.Contain("Communication goals:"));Assert.That(text,Does.Not.Contain("voice_only"));Assert.That(text,Does.Not.Contain("floating_orb"));Assert.That(text,Does.Not.Contain("humanoid_agent"));Click("PilotTaskContinueButton");yield return new WaitForSecondsRealtime(1f);var coordinator=ActiveObject("SceneTalkVR.Core.PilotCollectionSessionCoordinator, Assembly-CSharp");var task=Get(coordinator,"CurrentTask");var orchestrator=ActiveObject("SceneTalkVR.Runtime.SceneTalkOrchestrator, Assembly-CSharp");var payload=Get(orchestrator,"LastScenePayload");Assert.That(Get(task,"taskId"),Is.EqualTo("pilot_restaurant_walk_in"));Assert.That(Get(payload,"taskType"),Is.EqualTo("pilot_restaurant_walk_in"));Assert.That(Get(payload,"dialogueReply"),Is.EqualTo("Good evening! Welcome to Riverside Restaurant. Do you have a reservation?"));}
        ...
        {Create();AssertExitOverlay();Click("BeginPilotButton");yield return null;AssertExitOverlay();var phrases=new[]{new[]{"No, I don't have a reservation.","There are four of us.","Do you have a table available?","How long is the wait?"},new[]{"What do you recommend?","I'd like the grilled chicken.","I don't eat seafood.","I'd like a glass of water."},new[]{"This isn't what I ordered.","I ordered the beef pasta.","Could you replace it, please?","How long will the replacement take?"}};for(var condition=0;condition<3;condition++){Click("PilotTaskContinueButton");yield return null;AssertExitOverlay();foreach(var phrase in phrases[condition])EvaluatePilot(phrase);yield return new WaitForSecondsRealtime(1f);Assert.That(Active("PilotQuestionnairePanel"),Is.True);AssertExitOverlay();foreach(var item in new[]{"pilot_rc_01","pilot_sc_01","pilot_accept_01"})Click(item+"_4");Click("PilotQuestionnaireSubmitButton");yield return null;if(condition<2){Assert.That(Active("PilotNeutralTransitionPanel"),Is.True);AssertExitOverlay();Click("PilotTransitionContinueButton");yield return null;AssertExitOverlay();}}Assert.That(Active("PilotFinalRankingPanel"),Is.True);AssertExitOverlay();Click("VoiceOnlyRank1");Click("FloatingOrbRank2");Click("HumanoidAgentRank3");Click("VoiceOnlyPreferred");Input("PilotRankingReason").text="The feedback was easiest to follow.";Click("PilotRankingSubmitButton");yield return null;Assert.That(Active("PilotExperimentCompletionPanel"),Is.True);AssertExitOverlay();var coordinator=ActiveObject("SceneTalkVR.Core.PilotCollectionSessionCoordinator, Assembly-CSharp");var exportArgs=new object[]{null};Assert.That((bool)coordinator.GetType().GetMethod("ExportBundle").Invoke(coordinator,exportArgs),Is.True,exportArgs[0] as string);var auditArgs=new object[]{null};Assert.That((bool)coordinator.GetType().GetMethod("AuditBundle").Invoke(coordinator,auditArgs),Is.True,auditArgs[0] as string);}
        [UnityTest]public IEnumerator GlobalExit_AbortsAndPersistsPilotCollectionSession()
        {
            Create();Click("BeginPilotButton");yield return null;Click("PilotTaskContinueButton");yield return null;
            ...
            Assert.That(Active("InitialPanel"),Is.True);Assert.That((bool)Get(coordinator,"IsArmed"),Is.False);Assert.That(Get(coordinator,"Stage").ToString(),Is.EqualTo("None"));
            Assert.That(Get(current,"status").ToString(),Is.EqualTo("Aborted"));
            var events=System.IO.File.ReadAllText(System.IO.Path.Combine(dataFolder,"pilot_collection_operator_events.jsonl"));
            Assert.That(events,Does.Contain("PilotSessionExited"));Assert.That(events,Does.Contain("\"actor\":\"participant\""));
        }
        [UnityTest]public IEnumerator PicoDeviceValidation_CompletesPilotWithoutCollectionEligibility()
        {
            ...
            Click("BeginPilotButton");yield return null;
            for(var condition=0;condition<3;condition++){Click("PilotTaskContinueButton");yield return null;CompletePilotGoalsForQa(coordinator);yield return new WaitForSecondsRealtime(1f);foreach(var item in new[]{"pilot_rc_01","pilot_sc_01","pilot_accept_01"})Click(item+"_4");Click("PilotQuestionnaireSubmitButton");yield return null;if(condition<2){Click("PilotTransitionContinueButton");yield return null;}}
