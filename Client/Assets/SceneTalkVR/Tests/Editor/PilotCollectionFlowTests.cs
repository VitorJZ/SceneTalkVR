...
namespace SceneTalkVR.Tests.Editor
{
    public sealed class PilotCollectionFlowTests
    {
        ...
        [Test]public void CollectionAssignment_UsesWalkInTaskForEveryEmbodiment()
        {var allocator=new PilotAssignmentAllocator();allocator.TryCreateCollection("P-200","S",protocol,tasks,presentations,"resource",out var value,out _);Assert.That(value.conditions.All(x=>x.task.taskId==PilotAssignmentAllocator.TemporaryTaskId),Is.True);CollectionAssert.AreEquivalent(Enum.GetValues(typeof(PilotEmbodimentCondition)),value.conditions.Select(x=>x.embodimentCondition));Assert.That(value.conditions.Select(x=>x.task.taskAssignmentId).Distinct().Count(),Is.EqualTo(3));}
        ...
        [Test]public void TemporaryWalkInAssignment_InvalidatesPreviousPilotAssignments()
        {var old=new PilotAssignment{pilotProtocolVersion=protocol.ProtocolVersion,pilotAssignmentVersion="2.0-participant-choice",taskCatalogVersion=tasks.CatalogVersion};Assert.That(PilotAssignmentAllocator.IsCompatible(old,protocol.ProtocolVersion,tasks.CatalogVersion,out var error),Is.False);Assert.That(error,Is.EqualTo("pilot_assignment_version_changed"));}
