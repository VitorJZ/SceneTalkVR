...
namespace SceneTalkVR.Tests.Editor
{
    public sealed class RehearsalModeTests
    {
        ...
        [Test] public void T39_VoicesResolve() { Assert.That(voices.ValidateForRehearsal(out var e),Is.True,e);Assert.That(voices.TryGet("rehearsal_dialogue_voice",out var dialogue),Is.True);Assert.That(dialogue.voiceId,Is.EqualTo("101050"));Assert.That(voices.TryGet("rehearsal_feedback_voice",out var feedback),Is.True);Assert.That(feedback.voiceId,Is.EqualTo("default_female_en"));Assert.That(voices.Profiles.All(x=>x.sampleRate==24000),Is.True); }
